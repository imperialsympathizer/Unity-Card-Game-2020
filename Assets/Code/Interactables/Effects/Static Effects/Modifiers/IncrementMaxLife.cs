...
public class IncrementMaxLife : Modifier {
    // Modifier for incrementing character max life
    ...
    public IncrementMaxLife(int effectCount, List<Trigger> triggers, int incrementTimes = -1) : base(effectCount, triggers, 0) {
        this.incrementTimes = incrementTimes;
    }

    protected override void ResolveEffect(Trigger trigger) {
        if (character is Fighter fighter) {
            fighter.UpdateMaxLife(effectCount);
        }
