...
public class IncrementWillValue : Modifier {
    // Modifier for incrementing character will value
    ...
    public IncrementWillValue(int effectCount, List<Trigger> triggers, int incrementTimes = -1) : base(effectCount, triggers, 0) {
        this.incrementTimes = incrementTimes;
    }

    protected override void ResolveEffect(Trigger trigger) {
        if (character is Player player) {
            player.UpdateWillValue(effectCount);
        }
